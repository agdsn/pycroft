...
def complete_fints_challenge(
...
) -> tuple[FinTS3PinTanClient, bytes]:
    ...
    response = NeedRetryResponse.from_data(challenge)
    assert isinstance(
        response, NeedTANResponse
    ), "reconstructed NeedRetryResponse is not an instance of NeedTANResponse"

    with client.resume_dialog(dialog_data):
        client.send_tan(response, form.tan.data)
        dialog_data_out = client.pause_dialog()

    return client, dialog_data_out
...
def bank_accounts_import_run(bank_account_id: int) -> ResponseReturnValue:
    ...
    try:
        with fints_client.resume_dialog(fints_dialog_data), flash_fints_errors():
            ...
            dialog_data = fints_client.pause_dialog()
    ...
    fints_client.resume_dialog(dialog_data)
