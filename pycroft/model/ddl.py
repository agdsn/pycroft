...
def compile_if_clause(compiler: Compiled, clause: t.Any) -> t.Any:
    if isinstance(clause, ClauseElement):
        return str(
            clause.compile(
                compile_kwargs={"literal_binds": True}, dialect=compiler.dialect
            )
        )
...
def visit_create_function(
    element: CreateFunction, compiler: Compiled, **kw: t.Any
) -> str:
    """
...
class Rule(schema.DDLElement):
    ...
    def __init__(
        ...
        if isinstance(command_or_commands, Iterable) and not isinstance(
            command_or_commands, str
        ):
...
class DropRule(schema.DDLElement):
    ...
    def __init__(
        self, rule: Rule, if_exists: bool = False, cascade: bool = False
    ) -> None:
        """
        :param rule:
        :param if_exists:
...
class DropTrigger(schema.DDLElement):
    ...
    def __init__(
        self, trigger: Trigger, if_exists: bool = False, cascade: bool = False
    ) -> None:
        self.trigger = trigger
        self.if_exists = if_exists
        self.cascade = cascade
...
class CreateView(schema.DDLElement):
    def __init__(
        self, view: View, or_replace: bool = False, if_not_exists: bool = False
    ) -> None:
        self.view = view
        self.or_replace = or_replace
...
class DropView(schema.DDLElement):
    def __init__(
        self, view: View, if_exists: bool = False, cascade: bool = False
    ) -> None:
        self.view = view
        self.if_exists = if_exists
        self.cascade = cascade
...
def visit_create_view(element: CreateView, compiler: Compiled, **kw: t.Any) -> str:
    view = element.view
    opt_or_replace = (
        "OR REPLACE" if element.or_replace and not view.materialized else None
    )
...
class DDLManager:
    ...
    def add(
        ...
        if dialect:
            create_ddl = t.cast(
                schema.DDLElement, create_ddl.execute_if(dialect=dialect)
            )
    ...
    def add_function(
        self, table: Table, func: Function, dialect: str | None = None
    ) -> None:
        self.add(
            table,
    ...
    def add_trigger(
        self, table: Table, trigger: Trigger, dialect: str | None = None
    ) -> None:
        self.add(
            table,
